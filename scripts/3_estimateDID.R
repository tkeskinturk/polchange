...
rm(list = ls()) # clean-up
pacman::p_load(tidyverse, did, purrr, broom, parallel, Matrix)
...
set.seed(1123) ## set seed for the bootstrapped intervals
model_alt1 <- mclapply(l,
                       sensitive_did1,
                       mc.cores = 8)
d <- d |> mutate(model_alt1 = model_alt1)
...
set.seed(1123) ## set seed for the bootstrapped intervals
model_alt2 <- mclapply(l,
                       sensitive_did2,
                       mc.cores = 8)
d <- d |> mutate(model_alt2 = model_alt2)
